...
function ProcessTimeline({ detail }) {
  const stages = [
    ...
    { key: "关闭", label: "已关闭", action: "review_approve", hint: "待处理" },
  ];
    ...
    <ProcessPanel>
      ...
      <ProcessSteps>
        {stages.map((stage, index) => {
          const log = detail?.logs?.find((item) => item.action === stage.action);
...
function ActionDialog({ action, operators, busy, onClose, onSubmit }) {
  ...
  return (
    <ModalBackdrop role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <Dialog role="dialog" aria-modal="true" aria-label={titles[action]}>
        <DialogHeader><strong>{titles[action]}</strong><IconButton onClick={onClose} aria-label="关闭"><X size={17} /></IconButton></DialogHeader>
        <DialogBody>
          {action === "dispatch" ? <>
            <Field><label>处理人员</label><select value={form.assignee_id} onChange={(event) => setForm({ ...form, assignee_id: event.target.value })}>
              {operators.map((item) => <option key={item.id} value={item.id}>{item.name} / {item.department} / {item.position}</option>)}
            </select></Field>
            <FieldRow>
          ...
          </> : <Field><label>{requiresComment ? "处理说明" : "备注"}</label><textarea autoFocus value={form.comment} placeholder="请填写本次操作依据和现场情况" onChange={(event) => setForm({ ...form, comment: event.target.value })} /></Field>}
        </DialogBody>
        <DialogFooter><SecondaryButton onClick={onClose}>取消</SecondaryButton><PrimaryButton disabled={busy || (requiresComment && !form.comment.trim()) || (action === "dispatch" && !form.assignee_id)} onClick={submit}>{busy ? "提交中..." : "确认提交"}</PrimaryButton></DialogFooter>
      </Dialog>
    </ModalBackdrop>
  );
}
...
function AlarmCenter() {
  ...
  const performAction = async (action, values = {}) => {
    ...
    try {
      ...
      const payload = await response.json();
      setDetail(payload); setDialogAction(null); setToast("告警状态已更新"); setRefreshToken((value) => value + 1);
  ...
  const renderActions = () => {
    ...
    if (detail.status === "新建") return <><SecondaryButton onClick={() => setDialogAction("mark_false_positive")}><XCircle size={15} />标记误报</SecondaryButton><PrimaryButton onClick={() => performAction("confirm")} disabled={isActionLoading}><CheckCircle2 size={15} />确认告警</PrimaryButton></>;
    if (detail.status === "确认") return <><SecondaryButton onClick={() => setDialogAction("close")}>关闭事件</SecondaryButton><PrimaryButton onClick={() => setDialogAction("dispatch")}><Send size={15} />派发处理</PrimaryButton></>;
    if (detail.status === "处理中") return <PrimaryButton onClick={() => setDialogAction("submit_feedback")}><ClipboardCheck size={15} />提交反馈</PrimaryButton>;
    if (detail.status === "待复核") return <><SecondaryButton onClick={() => setDialogAction("review_reject")}>驳回重办</SecondaryButton><PrimaryButton onClick={() => setDialogAction("review_approve")}><CheckCircle2 size={15} />复核通过</PrimaryButton></>;
        ...
        <DetailPanel>
          {isDetailLoading ? <Centered>告警详情加载中</Centered> : detailError ? <Centered $error>{detailError}</Centered> : detail ? <>
            ...
            <DetailScroll>
              ...
              {detail.assignment && <DetailSection><h3>当前派单</h3><Assignment><UserRound size={16} /><div><strong>{detail.assignment.assignee_name || "待接单"}</strong><span>{detail.assignment.department} · {PRIORITY_LABELS[detail.assignment.priority]}优先级</span></div></Assignment><p>{detail.assignment.instruction}</p>{detail.assignment.feedback && <Feedback>现场反馈：{detail.assignment.feedback}</Feedback>}</DetailSection>}
