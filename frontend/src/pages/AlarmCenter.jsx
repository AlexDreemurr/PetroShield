...
import {
  ...
  getCachedAlarmList,
  getCachedAlarmOperators,
  getWarmAlarmList,
  invalidateAlarmCenterCache,
  loadAlarmDetail,
  loadAlarmList,
  loadAlarmOperators,
  primeAlarmDetail,
...
function StatusBadge({ status }) {
  return (
    <Status $status={status}>{status === "新建" ? "待处理" : status}</Status>
  );
}
...
function ProcessTimeline({ detail }) {
  const stages = [
    ...
    },
    {
      ...
      alternateAction: "close",
      hint: "待处理",
    },
  ];
  const isFalsePositive = detail?.status === "误报";
  const matchedIndex = stages.findIndex((stage) => stage.key === detail?.status);
  const currentIndex = Math.max(0, matchedIndex);
  const progress = isFalsePositive
    ? 0
  ...
  return (
    <ProcessPanel>
      <ProcessHeading>
        ...
        {isFalsePositive && <ProcessOutcome>已标记误报，流程终止</ProcessOutcome>}
      </ProcessHeading>
      <ProcessSteps>
        <ProcessRail>
          <ProcessRailProgress style={{ width: `${progress}%` }} />
        </ProcessRail>
        {stages.map((stage, index) => {
          const log = detail?.logs?.find(
            (item) =>
              item.action === stage.action ||
              item.action === stage.alternateAction
          );
          const state = isFalsePositive
            ? index === 0
              ? "stopped"
              : "pending"
            : index < currentIndex
              ...
              : index === currentIndex
                ...
                : "pending";
          return (
            <ProcessStep key={stage.key} $state={state}>
              <StepDot $state={state}>
              ...
              <StepCopy>
                ...
                <span>
                  {log
                    ? formatDateTime(log.create_time, false)
                    : state === "current"
                      ? "当前节点"
                      : stage.hint}
                </span>
...
function ActionDialog({ action, operators, busy, onClose, onSubmit }) {
  ...
  const submit = () => {
    const payload = { ...form };
    if (action === "dispatch")
      payload.department = selectedOperator?.department ?? form.department;
      ...
      <Dialog role="dialog" aria-modal="true" aria-label={actionConfig.title}>
        ...
        <DialogBody>
          ...
          {action === "dispatch" ? (
              ...
              <FieldRow>
                ...
                <Field>
                  ...
                  <input
                    ...
                    value={form.due_time}
                    onChange={(event) =>
                      setForm({ ...form, due_time: event.target.value })
                    }
                  />
                </Field>
              </FieldRow>
            ...
            <>
              ...
              {action === "submit_feedback" && (
                <Field>
                  <label>证据补充说明（选填）</label>
                  <input
                    value={form.evidence_note}
                    placeholder="例如：现场照片已由班组留档"
                    onChange={(event) =>
        ...
        <DialogFooter>
          ...
          <PrimaryButton
            disabled={
              busy ||
              (requiresComment && !form.comment.trim()) ||
              (action === "dispatch" &&
                (!form.assignee_id ||
...
function AlarmCenter() {
  ...
  const emptyFilters = useMemo(
    () => ({
      ...
      level: "",
      status: "",
      keyword: linkedAlarmId,
    }),
    [linkedAlarmId, today, weekAgo]
  );
  const initialQuery = buildQuery(emptyFilters, 1);
  const initialListData =
    getCachedAlarmList(initialQuery) ?? getWarmAlarmList(linkedAlarmId);
  const initialDetail = linkedAlarmId
    ? getCachedAlarmDetail(linkedAlarmId)
    : null;
  ...
  const [selectedId, setSelectedId] = useState(linkedAlarmId || null);
  const [detail, setDetail] = useState(initialDetail);
  const [operators, setOperators] = useState(getCachedAlarmOperators() ?? []);
  const [isListLoading, setIsListLoading] = useState(!initialListData);
  const [isDetailLoading, setIsDetailLoading] = useState(
    Boolean(linkedAlarmId && !initialDetail)
  );
  ...
  const loadList = useCallback(
    async (signal) => {
      ...
      const cachedList =
        getCachedAlarmList(query) ??
        (filters.keyword ? getWarmAlarmList(filters.keyword) : null);
      if (cachedList) setListData(cachedList);
      setIsListLoading(!cachedList);
      setListError("");
      try {
        const payload = await loadAlarmList(query, { force: Boolean(cachedList) });
        ...
        if (error.name !== "AbortError") {
          if (!cachedList) {
            setListData((current) => ({ ...current, items: [], total: 0 }));
  ...
  useEffect(() => {
    let active = true;
    const cachedOperators = getCachedAlarmOperators();
    loadAlarmOperators({ force: Boolean(cachedOperators) })
      .then((data) => {
        if (active) setOperators(data.items ?? []);
      })
      .catch(() => {
        if (active && !cachedOperators) setOperators([]);
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const cachedDetail = getCachedAlarmDetail(selectedId);
    setIsDetailLoading(!cachedDetail);
    setDetailError("");
    setDetail(cachedDetail);
    loadAlarmDetail(selectedId, { force: Boolean(cachedDetail) })
      .then((payload) => {
  ...
  return (
    <Page>
      <PageHeader>
        ...
        <HeaderActions>
          <MenuWrap>
            <ToolButton onClick={() => setColumnMenuOpen((value) => !value)}>
              <Columns3 size={16} />
              列设置
            </ToolButton>
            {columnMenuOpen && (
      ...
      <Filters>
        ...
        <FilterField>
          ...
          <select
            ...
            onChange={(event) => updateFilter("alarm_type", event.target.value)}
          >
        ...
        <FilterField>
          ...
          >
            <option value="">全部</option>
            {listData.options.statuses.map((item) => (
              <option key={item}>{item}</option>
            ))}
          </select>
        </FilterField>
      ...
      <Workspace>
        <ListPanel>
          <ListSummary>
            共 <strong>{listData.total}</strong> 条
          </ListSummary>
          <AlarmTable $visible={visibleColumns.join(" ")}>
            <TableHeader>
              {ALL_COLUMNS.filter((item) =>
            ...
            <TableBody>
              {isListLoading ? (
                <Centered>告警数据加载中</Centered>
              ) : listError ? (
                <Centered $error>{listError}</Centered>
              ) : listData.items.length === 0 ? (
                <Centered>暂无符合条件的告警</Centered>
              ) : (
                listData.items.map((item) => (
                  ...
                  >
                    ...
                    {visibleColumns.includes("subject") && (
                      <span title={item.subject?.name}>
                        {item.subject?.name ?? item.area?.name ?? "--"}
                      </span>
                    )}
        ...
        <MapPanel>
          ...
          <AlarmCenterMap
            detail={detail}
            isDataLoading={isDetailLoading}
            hasDataError={Boolean(detailError)}
            mapType={mapType}
          />
          <MapLegend>
            <span>
        ...
        <DetailPanel>
          {isDetailLoading ? (
            <Centered>告警详情加载中</Centered>
          ) : detailError ? (
            <Centered $error>{detailError}</Centered>
          ) : detail ? (
            <>
              ...
              <DetailScroll>
                <InfoRows>
                  ...
                  <InfoRow>
                    <span>人员/设备</span>
                    <strong>{detail.subject?.name ?? "未关联"}</strong>
                  </InfoRow>
                  <InfoRow>
                    <span>发生时间</span>
                    <strong>{formatDateTime(detail.time)}</strong>
                  </InfoRow>
                  <InfoRow>
                    <span>所在区域</span>
                    <strong>{detail.area?.name ?? "未标注区域"}</strong>
                  </InfoRow>
                  <InfoRow>
                    <span>AI置信度</span>
                    <strong>
                ...
                <DetailSection>
                  ...
                  {detail.status === "新建" ? (
                    <AdvicePending>
                      ...
                      <div>
                        <strong>等待确认告警</strong>
                        <span>确认后，系统将结合告警、区域和关联对象生成处置建议。</span>
                      </div>
                    </AdvicePending>
                  ) : (
                    <Advice>
                ...
                {detail.assignment && (
                  <DetailSection>
                    ...
                    <Assignment>
                      ...
                      <div>
                        <strong>
                          {detail.assignment.assignee_name || "待接单"}
                        </strong>
                        <span>
                          {detail.assignment.department} ·{" "}
                          {PRIORITY_LABELS[detail.assignment.priority]}优先级 ·{" "}
                          {ASSIGNMENT_STATUS_LABELS[detail.assignment.status] ??
                            detail.assignment.status}
                        </span>
                        <span>
                          要求完成：
                          {formatDateTime(detail.assignment.due_time, false)}
                        </span>
                      </div>
                    </Assignment>
                    <p>{detail.assignment.instruction}</p>
                    {detail.assignment.feedback && (
                      <Feedback>
                        现场反馈：{detail.assignment.feedback}
                      </Feedback>
                    )}
                  </DetailSection>
                )}
                <DetailSection>
                  ...
                  <ResourceTags>
                    {detail.resources?.length ? (
                      detail.resources.slice(0, 5).map((item) => (
                        <span key={item.id}>
                          {item.type?.includes("摄像") ? (
                            <Camera size={13} />
                          ) : (
                            <CircleDot size={13} />
                          )}
                ...
                <DetailSection>
                  ...
                  <VideoPreview>
                    <Camera size={30} />
                    <button aria-label="播放关联视频">
                      <Play size={20} fill="currentColor" />
                    </button>
                    <span>
                      {detail.resources?.find((item) =>
                        item.type?.includes("摄像")
                      )?.name ?? "暂无可用视频流"}
                    </span>
                  </VideoPreview>
                        ...
                        .map((item) => (
                          <li key={item.id}>
                            ...
                            <div>
                              ...
                              <span>
                                {item.operator_name} ·{" "}
                                {formatDateTime(item.create_time, false)}
                              </span>
      ...
      {dialogAction && (
        <ActionDialog
          ...
          onSubmit={(values) => performAction(dialogAction, values)}
        />
      )}
...
const Page = styled.div`
  ...
  background: #f8fafc;
  display: grid;
  grid-template-rows: 34px 44px minmax(0, 1fr);
  gap: 8px;
  font-size: 12px;
`;
...
const Workspace = styled.div`
  min-height: 0;
  display: grid;
  grid-template-columns: 460px minmax(500px, 1fr) 390px;
...
`;
const AlarmTable = styled.div`
  min-height: 0;
  display: grid;
  grid-template-rows: 34px minmax(0, 1fr);
  --cols: ${(p) =>
    [
      p.$visible.includes("time") && "122px",
      p.$visible.includes("type") && "94px",
      p.$visible.includes("level") && "70px",
      p.$visible.includes("subject") && "minmax(70px,1fr)",
      p.$visible.includes("status") && "62px",
    ]
      .filter(Boolean)
      .join(" ")};
`;
...
const TableBody = styled.div`
  ...
  overflow: hidden;
  display: grid;
  grid-template-rows: repeat(${PAGE_SIZE}, minmax(0, 1fr));
...
const TableRow = styled.button`
  ...
  box-shadow: ${(p) => (p.$selected ? "inset 3px 0 #1677ff" : "none")};
  text-align: left;
  cursor: pointer;
  font-size: 10px;
  overflow: hidden;
...
`;
const AdvicePending = styled.div`
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 11px;
...
`;
const AdvicePendingIcon = styled.span`
  width: 30px;
  height: 30px;
  flex: 0 0 auto;
  display: grid;
...
const VideoPreview = styled.div`
  ...
  overflow: hidden;
  color: #cbd5e1;
  background: linear-gradient(135deg, #27364a, #111827 55%, #344054);
  border-radius: 4px;
  > button {
    position: absolute;
    width: 38px;
...
`;
const TerminalState = styled.span`
  display: inline-flex;
  align-items: center;
  gap: 6px;
  color: #28855b;
`;
const ProcessPanel = styled(Panel)`
  grid-column: 1 / 3;
  grid-row: 2;
...
const ProcessTitle = styled.h2`
  font-size: 12px;
`;
const ProcessOutcome = styled.span`
  color: #b54708;
  font-size: 10px;
  font-weight: 600;
`;
...
const ProcessRail = styled.div`
  ...
  background: #e3e8ef;
`;
const ProcessRailProgress = styled.span`
  display: block;
  height: 100%;
  background: #69a8f8;
...
const StepDot = styled.span`
  ...
  border-radius: 50%;
  color: inherit;
  background: ${(p) => (p.$state === "current" ? "#eaf3ff" : "#fff")};
  font-weight: 700;
`;
