...
import { loadRuntimeDictionaries } from "../services/runtimeDictionaries";

const EMPTY_ITEM = { code: "", value: "", name: "", color: "#2563eb", order: 10, status: "active", remark: "" };

function ItemDialog({ item, onClose, onSubmit, busy }) {
  const [form, setForm] = useState(item ?? EMPTY_ITEM);
  const [error, setError] = useState("");
  const update = (key) => (event) => setForm((current) => ({ ...current, [key]: event.target.value }));
  function submit(event) { event.preventDefault(); if (!form.code.trim() || !form.value.trim() || !form.name.trim()) { setError("请填写字典编码、业务值和显示名称"); return; } onSubmit({ ...form, code: form.code.trim().toUpperCase(), value: form.value.trim(), name: form.name.trim(), order: Number(form.order) || 0 }); }
  return <Backdrop onMouseDown={busy ? undefined : onClose}><Dialog onMouseDown={(event) => event.stopPropagation()}><DialogHeader><div><h2>{item ? "编辑字典项" : "新增字典项"}</h2><p>业务值负责关联真实数据，显示名称和颜色负责界面呈现</p></div><IconButton type="button" title="关闭" disabled={busy} onClick={onClose}><X size={17} /></IconButton></DialogHeader><DialogForm onSubmit={submit}><FormGrid><Field><span>字典编码</span><input value={form.code} disabled={Boolean(item)} onChange={update("code")} placeholder="例如 DEVICE_OFFLINE" /></Field><Field><span>业务值</span><input value={form.value} disabled={Boolean(item)} title={item ? "业务值是数据绑定键，创建后不可修改" : undefined} onChange={update("value")} placeholder="例如 offline 或 设备离线" /></Field><Field><span>显示名称</span><input value={form.name} onChange={update("name")} placeholder="请输入显示名称" /></Field><Field><span>显示颜色</span><ColorInput><input type="color" value={form.color} onChange={update("color")} /><input value={form.color} onChange={update("color")} /></ColorInput></Field><Field><span>排序值</span><input type="number" min="0" value={form.order} onChange={update("order")} /></Field><Field><span>状态</span><select value={form.status} onChange={update("status")}><option value="active">启用</option><option value="disabled">停用</option></select></Field><Field $wide><span>说明</span><input value={form.remark} onChange={update("remark")} placeholder="请输入业务含义或使用范围" /></Field></FormGrid>{error ? <ErrorText>{error}</ErrorText> : null}<DialogActions><SecondaryButton type="button" disabled={busy} onClick={onClose}>取消</SecondaryButton><PrimaryButton type="submit" disabled={busy}>{busy ? "保存中..." : "保存"}</PrimaryButton></DialogActions></DialogForm></Dialog></Backdrop>;
}
...
function DataDictionary() {
  ...
  const items = useMemo(() => (selectedGroup?.items ?? []).filter((item) => !keyword.trim() || [item.code, item.value, item.name, item.remark].some((value) => String(value || "").toLowerCase().includes(keyword.trim().toLowerCase()))).sort((a, b) => a.order - b.order), [keyword, selectedGroup]);
  ...
  async function saveItem(form) {
    ...
    try {
      ...
      const response = await apiFetch(editing ? `${API_BASE_URL}/system/dictionaries/items/${dialogItem.id}` : `${API_BASE_URL}/system/dictionaries/${selectedGroup.id}/items`, {
        ...
        body: JSON.stringify(editing ? { value: form.value, name: form.name, color: form.color, order: form.order, status: form.status, remark: form.remark || null } : form),
      });
      if (!response.ok) throw new Error(await readApiError(response, "字典项保存失败"));
      setDialogItem(undefined); notify(editing ? "字典项已更新并应用" : "字典项已新增并应用"); await load(); await loadRuntimeDictionaries({ force: true }).catch(() => undefined);
  ...
  async function toggleItem(target) {
    setBusy(true);
    try {
      const response = await apiFetch(`${API_BASE_URL}/system/dictionaries/items/${target.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ value: target.value, name: target.name, color: target.color, order: target.order, status: target.status === "active" ? "disabled" : "active", remark: target.remark || null }) });
      if (!response.ok) throw new Error(await readApiError(response, "状态修改失败"));
      notify(target.status === "active" ? "字典项已停用并从业务界面移除" : "字典项已启用并应用"); await load(); await loadRuntimeDictionaries({ force: true }).catch(() => undefined);
    } catch (nextError) { notify(nextError.message); }
    finally { setBusy(false); }
  }
...
const Table = styled.table`width:100%;border-collapse:collapse;table-layout:fixed;font-size:12px;th{position:sticky;top:0;z-index:1;height:38px;padding:0 10px;text-align:left;background:hsl(216 28% 97%);color:hsl(218 12% 42%);font-weight:600;border-bottom:1px solid hsl(218 18% 88%)}td{height:52px;padding:0 10px;border-bottom:1px solid hsl(218 18% 92%);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}th:nth-child(1){width:6%}th:nth-child(2){width:14%}th:nth-child(3){width:13%}th:nth-child(4){width:12%}th:nth-child(5){width:11%}th:nth-child(6){width:8%}th:nth-child(7){width:19%}th:nth-child(8){width:17%}tbody tr:hover{background:hsl(216 55% 98%)}`;
const Code = styled.code`font-family:var(--font-data);font-size:11px;color:hsl(220 65% 40%);`;
const BusinessValue = styled.code`font-family:var(--font-data);font-size:11px;color:hsl(218 16% 34%);`;
